...
const { computeLabelLayout, LABEL_GAP } = require('./labelLayout');
...
// Poll GetSceneItemTransform until OBS has rasterized the text source and
// reports non-zero bounds (up to 5 × 100 ms). Returns { width, height } from
// sourceWidth/sourceHeight; falls back to { width: 0, height: 0 } so that
// computeLabelLayout uses its font-metric estimate instead.
async function measureTextItem(obsClient, sceneName, sceneItemId) {
  for (let i = 0; i < 5; i++) {
    const { sceneItemTransform } = await obsClient.call('GetSceneItemTransform', {
      sceneName,
      sceneItemId,
    });
    const w = sceneItemTransform.sourceWidth || 0;
    const h = sceneItemTransform.sourceHeight || 0;
...
async function createStreamGroupV2(groupName, streamName, teamName, url, opts = {}) {
  ...
  try {
    ...
    // Conservative team-box height estimate used to anchor the streamer label Y
    // even when the team items are not found or their positioning throws. Computed
    // with zero measurements so it is pure and cheap; the real measured layout
    // (inside the try below) overwrites teamLayoutBoxHeight when available.
    let teamLayoutBoxHeight = computeLabelLayout({
      measuredWidth: 0,
      measuredHeight: 0,
      textLength: teamName.length,
      fontSize: 96,
      boxTopY: 10,
    }).boxHeight;
    ...
    if (sourceItem && textSourceItem && colorSourceItem) {
      try {
        // Initial placement so OBS rasterizes the text source and we can measure it.
        await obsClient.call('SetSceneItemTransform', {
          sceneName: streamGroupName,
          sceneItemId: textSourceItem.sceneItemId,
          sceneItemTransform: {
        ...
        // Poll until OBS reports non-zero bounds, then derive box geometry.
        const { width: measuredW, height: measuredH } = await measureTextItem(
          obsClient, streamGroupName, textSourceItem.sceneItemId
        );
        ...
        teamLayoutBoxHeight = teamLayout.boxHeight;

        await obsClient.call('SetInputSettings', {
          inputName: colorSourceName,
          inputSettings: { width: teamLayout.boxWidth, height: teamLayout.boxHeight },
        });

        // Final position: center both text and color box at the computed centerY.
        await obsClient.call('SetSceneItemTransform', {
          sceneName: streamGroupName,
          sceneItemId: textSourceItem.sceneItemId,
          sceneItemTransform: {
            positionX: teamLayout.centerX,
            positionY: teamLayout.centerY,
            scaleX: 1.0,
            scaleY: 1.0,
            alignment: 0,
          },
    ...
    // Streamer box top = team box bottom + LABEL_GAP; both labels are centered
    // within their color boxes using geometry derived from measured text bounds.
    const streamerBoxTopY = 10 + teamLayoutBoxHeight + LABEL_GAP;
    ...
    if (nameTextItem && nameColorItem) {
      try {
        ...
        // Initial placement so OBS rasterizes the streamer text source.
        await obsClient.call('SetSceneItemTransform', {
          sceneName: streamGroupName,
          sceneItemId: nameTextItem.sceneItemId,
          sceneItemTransform: {
            positionX: 960,
            positionY: streamerBoxTopY,
            scaleX: 1.0,
            scaleY: 1.0,
            alignment: 0,
          },
        ...
        // Poll for measured bounds, then compute final layout.
        const { width: nameMeasuredW, height: nameMeasuredH } = await measureTextItem(
          obsClient, streamGroupName, nameTextItem.sceneItemId
        );
        const streamerLayout = computeLabelLayout({
          measuredWidth: nameMeasuredW,
          measuredHeight: nameMeasuredH,
          textLength: streamName.length,
          fontSize: 96,
        ...
        await obsClient.call('SetInputSettings', {
          ...
          inputSettings: { width: streamerLayout.boxWidth, height: streamerLayout.boxHeight },
        });
        ...
        await obsClient.call('SetSceneItemTransform', {
          ...
          sceneItemId: nameTextItem.sceneItemId,
          sceneItemTransform: {
            positionX: streamerLayout.centerX,
            positionY: streamerLayout.centerY,
            scaleX: 1.0,
            scaleY: 1.0,
            alignment: 0,
          },
...
async function deleteStreamComponents(streamName, teamName, groupName) {
  try {
    ...
    // 4b. Remove the per-stream streamer label (text + background). Unlike the
