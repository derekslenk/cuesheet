...
async function deleteStreamComponents(streamName, teamName, groupName) {
  try {
    ...
    // 4b. Remove the per-stream label inputs (streamer text + background,
    // per-stream team background, accent bar). Unlike the shared team text,
    // these are unique to the stream, so they must be removed — RemoveScene
    // above drops the scene items but not the underlying inputs.
...
async function deleteTeamComponents(teamName, groupName) {
  try {
    ...
    // label input for this team (streamer text/bg, per-stream team bg, accent).
    // RemoveScene drops scene items but not inputs, and these aren't
    // browser_source kind, so step 4 below never sweeps them.
    try {
      const { inputs } = await obsClient.call('GetInputList');
      const cleanGroupName = cleanObsName(groupName || teamName);
      const labelInputs = inputs.filter(input =>
        input.inputName === `${textSourceName}_bg` ||
        (input.inputName.startsWith(`${cleanGroupName}_`) && (
          input.inputName.endsWith('_name_text') ||
          input.inputName.endsWith('_name_text_bg') ||
          input.inputName.endsWith('_team_bg') ||
          input.inputName.endsWith('_accent')
        ))
      );
