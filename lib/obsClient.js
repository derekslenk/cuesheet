...
// settles, making computeLabelLayout fall back to its font-metric estimate.
...
async function createStreamGroupV2(groupName, streamName, teamName, url, opts = {}) {
  ...
  try {
    ...
    // Box height is fixed per font size, so this is exact even before measuring;
    // it anchors the streamer label Y even if the team block is skipped or throws.
    const teamLayoutBoxHeight = computeLabelLayout({
      measuredWidth: 0,
    ...
    if (sourceItem && textSourceItem && colorSourceItem) {
      try {
        ...
        // Poll until OBS reports a stable width, then derive box geometry.
        const measuredTeamWidth = await measureTextWidth(
          obsClient, streamGroupName, textSourceItem.sceneItemId
        );
        const teamLayout = computeLabelLayout({
          measuredWidth: measuredTeamWidth,
          textLength: teamName.length,
          fontSize: 96,
          boxTopY: 10,
        });
    ...
    if (nameTextItem && nameColorItem) {
      try {
        ...
        // Poll for a stable width, then compute final layout.
        const measuredNameWidth = await measureTextWidth(
          obsClient, streamGroupName, nameTextItem.sceneItemId
        );
        const streamerLayout = computeLabelLayout({
          measuredWidth: measuredNameWidth,
