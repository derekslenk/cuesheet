...
| `labelLayout.js` | Unified-plate label geometry + styling for stream overlays (CommonJS). Exports `computeUnifiedPlateLayout({ teamTextWidth, nameTextWidth, teamTextLength, nameTextLength })` (one plate: caps team line over bold streamer line + left accent bar), `buildTeamTextSettings`/`buildNameTextSettings` (Bahnschrift, no outline), and tuning constants (`PLATE_*`, `ACCENT_*`, `LABEL_VPAD`, `INNER_GAP`, font sizes, ABGR colors). `createStreamGroupV2` is the single owner of the label look: plate width from stable measured text widths (`measureTextWidth` polling, two equal consecutive reads) with font-metric fallback; heights fixed per font size. Team text input is shared per team, but its background (`*_team_bg`) and the accent (`*_accent`) are per-stream. |
