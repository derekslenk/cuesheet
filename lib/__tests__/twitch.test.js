...
describe('twitchLoginFromUrl', () => {
  ...
  it('returns null for reserved twitch site paths (not channels)', () => {
    expect(twitchLoginFromUrl('https://www.twitch.tv/directory')).toBeNull();
    expect(twitchLoginFromUrl('https://www.twitch.tv/videos/123456')).toBeNull();
    expect(twitchLoginFromUrl('https://twitch.tv/settings')).toBeNull();
    // a real channel that merely starts with similar letters is unaffected
    expect(twitchLoginFromUrl('https://twitch.tv/directormaximus')).toBe('directormaximus');
  });
});
...
describe('getViewerCount / getViewerCounts', () => {
  ...
  // The label fan-out (many overlay sources sharing channels) relies on batching
  // ≤100 logins/request; with >100 we expect ⌈N/100⌉ Helix calls. (T-M3 / T-H3)
  it('splits >100 logins into separate /streams requests of <=100 each', async () => {
    const calls = mockViewers({}); // none live — we only care about request batching
    const logins = Array.from({ length: 150 }, (_, i) => `u${i}`);

    await getViewerCounts(logins);

    const streamCalls = calls.filter((u) => u.includes('/streams'));
    expect(streamCalls.length).toBe(2); // ceil(150 / 100)
    const perCall = streamCalls
      .map((u) => (u.match(/user_login=/g) || []).length)
      .sort((a, b) => b - a);
    expect(perCall).toEqual([100, 50]); // batched 100 + 50, each <= 100
  });

  // An offline login is cached as null so it isn't re-queried every poll — the
  // anti-hammer mechanism. Deterministic via an explicit `now`. (T-H3)
  it('caches an offline login as null and does not re-query it within TTL', async () => {
    const calls = mockViewers({ live: 5 }); // "ghost" omitted from the response = offline

    const first = await getViewerCounts(['live', 'ghost'], 1000);
    expect(first.get('live')).toBe(5);
    expect(first.has('ghost')).toBe(false); // offline channels are omitted from the result
    const before = calls.filter((u) => u.includes('/streams')).length;

    // Second poll 10s later (within the 25s TTL): both served from cache, so no
    // new /streams request even though `ghost` has no count.
    const second = await getViewerCounts(['live', 'ghost'], 11_000);
    const after = calls.filter((u) => u.includes('/streams')).length;

    expect(after).toBe(before);
    expect(second.get('live')).toBe(5);
    expect(second.has('ghost')).toBe(false);
  });
  ...
  it('throws on a non-ok Helix response', async () => {
    ...
    await expect(getViewerCounts(['a'])).rejects.toThrow(/503/);
  });
});
