...
const SAVED = {
  ...
  LABEL_FPS: process.env.LABEL_FPS,
};
...
describe('label env helpers', () => {
  ...
  it('labelFps defaults to 15, honors LABEL_FPS, and clamps out-of-range/garbage', () => {
    expect(labelFps()).toBe(15);
    process.env.LABEL_FPS = '10';
    expect(labelFps()).toBe(10);
    process.env.LABEL_FPS = '999';
    expect(labelFps()).toBe(60);
    process.env.LABEL_FPS = '0';
    expect(labelFps()).toBe(1);
    process.env.LABEL_FPS = 'nonsense';
    expect(labelFps()).toBe(15);
  });
...
describe('createStreamLabelBrowserSource', () => {
  ...
  it('throws (no CreateInput) when streamId is missing — never points at /overlay/stream/undefined', async () => {
    const call = makeCall([]);
    await expect(
      createStreamLabelBrowserSource({ call }, { ...opts, streamId: undefined })
