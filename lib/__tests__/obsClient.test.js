...
describe('buildFfmpegPlaybackSettings (Studio Mode preview/program policy)', () => {
  ...
  const ORIGINAL_HW = process.env.OBS_HW_DECODE;
  afterEach(() => {
    if (ORIGINAL === undefined) delete process.env.OBS_RESTART_ON_ACTIVATE;
    else process.env.OBS_RESTART_ON_ACTIVATE = ORIGINAL;
    if (ORIGINAL_HW === undefined) delete process.env.OBS_HW_DECODE;
    else process.env.OBS_HW_DECODE = ORIGINAL_HW;
  });

  it('defaults hw_decode=true (GPU/NVDEC) when OBS_HW_DECODE unset', () => {
    delete process.env.OBS_HW_DECODE;
    expect(buildFfmpegPlaybackSettings().hw_decode).toBe(true);
  });

  it('honors OBS_HW_DECODE=false (software/CPU decode — trade VRAM for CPU/RAM)', () => {
    process.env.OBS_HW_DECODE = 'false';
    expect(buildFfmpegPlaybackSettings().hw_decode).toBe(false);
  });
...
describe('applyFfmpegPlaybackSettings (retrofit existing inputs)', () => {
  ...
  it('updates only live UDP sources and skips local-file media (overlay:true)', async () => {
    ...
    // The retrofit also mutes each updated live source — pre-policy sources
    // were created unmuted and audible on the broadcast tracks.
    const muteCalls = call.mock.calls.filter(([req]) => req === 'SetInputMute');
    expect(muteCalls.map(([, p]) => p)).toEqual([
      { inputName: 'team_a_main', inputMuted: true },
      { inputName: 'team_b_main', inputMuted: true },
    ]);
  ...
  it('updates nothing when every ffmpeg_source is a local file', async () => {
    ...
    expect(call.mock.calls.some(([req]) => req === 'SetInputMute')).toBe(false);
  });
...
describe('createStreamInput (OBS per-input choreography, V1+V2)', () => {
  ...
  it('uses ffmpeg_source for V2 and mutes it (Monitor Off hides audio locally but it still broadcasts)', async () => {
    const call = makeCall();
